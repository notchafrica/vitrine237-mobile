...
class SearchProvider {
  static Future<List<Map<String, String>>> findCity(q) async {
    List<Map<String, String>> list = [];
    var r = await API.get('/cities/find?q=' + q);

    if (r.statusCode != 200) {
      return [];
    }

    List map = jsonDecode(r.body);
    map.forEach((city) {
      list.add({'name': city['name'], 'slug': city['slug']});
    });

    return list;
  }

  static Future<List<Company>> search(String code, {city = ''}) async {
